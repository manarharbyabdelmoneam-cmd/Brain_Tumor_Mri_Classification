...
def display_upload_controls() -> Tuple[bool, bool, bool]:
    ...
    with col1:
        upload_clicked = st.button("📤 رفع صورة", use_column_width=True)
    
    with col2:
        clear_clicked = st.button("🗑️ مسح الكل", use_column_width=True)
    
    with col3:
        analyze_clicked = st.button("🔍 تحليل", use_column_width=True, type="primary")
